...
export default function CalmSpace() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    if (session?.confirmed) {
      ...
      selectTrack(session.selectedTrack);
      startSession(option, session.duration);
      setActiveType(null);
  ...
  }, [pauseSession, selectTrack, startSession]));
  ...
  const handleStart = (option: FocusSessionOption) => {
    ...
    router.push('/music-select' as any);
  };
  ...
  if (isRunning) {
    return (
      <ThemedScreen showNightEffects nightEffectIntensity={nightIntensity}>
        ...
        <View style={styles.runHeader}>
          ...
          <Text style={styles.modeLabel} numberOfLines={1}>{selectedOption?.label}</Text>
          <TouchableOpacity
            style={styles.musicBtn}
            onPress={() => router.push('/music-select' as any)}
          >
            <Text style={styles.musicIcon}>{!selectedTrack ? '🎵' : isPlaying ? '🔊' : '⏸'}</Text>
            <Text style={styles.musicLabel} numberOfLines={1}>
              {!selectedTrack ? 'Chọn nhạc' : isPlaying ? 'Đang phát' : 'Tạm dừng'}
            </Text>
          </TouchableOpacity>
        </View>

        <View style={styles.runBody}>
          <GrowingPlant progress={progress} status={plant?.status ?? 'GROWING'} flowerType={plant?.flowerType} />
          <FocusTimer
          ...
          <View style={styles.musicBar}>
            <TouchableOpacity style={styles.musicBarRow} onPress={togglePlayPause} activeOpacity={0.8}>
              ...
              <TouchableOpacity onPress={() => router.push('/music-select' as any)} style={styles.changeMusicBtn}>
                <Text style={styles.changeMusicText}>Đổi</Text>
              </TouchableOpacity>
            </TouchableOpacity>
            ...
            <View style={styles.volumeRow}>
              ...
              >
                ...
                <View style={[styles.sliderThumb, { left: `${Math.max(0, volume * 100 - 1)}%` as any }]} />
              </View>
  ...
  return (
    <ThemedScreen showNightEffects nightEffectIntensity="normal">
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.screenTitle}>Góc yên 🌿</Text>
        <View style={{ width: 80 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  runBody: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 28, paddingHorizontal: 24, paddingBottom: 8 },
  ...
  changeMusicBtn: { backgroundColor: COLORS.green[100], borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4 },
  changeMusicText: { fontSize: 12, color: COLORS.green.dark, fontWeight: '600' },
  ...
  runControls: { flexDirection: 'row', paddingHorizontal: 24, paddingBottom: Platform.OS === 'android' ? 48 : 36, paddingTop: 12, gap: 12 },
